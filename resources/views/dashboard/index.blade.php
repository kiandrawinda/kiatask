...
.mood-overlay.active { display: flex; }

/* Mood button selected state (pure CSS, no Alpine needed) */
.mood-btn.selected {
...
<div class="topbar">
    ...
    <div style="display:flex;align-items:center;gap:12px">
        @if(!$todayMood)
        <button onclick="openMoodModal()" class="btn btn-secondary" style="font-size:0.8rem">
            😊 Log Mood
        </button>
        @endif
    ...
    <div style="display:flex;flex-direction:column;gap:20px">
        ...
        <div class="mood-widget" id="mood-check">
            ...
            <div style="text-align:center;padding:8px">
                ...
                @if($todayMood->note)
                <div style="color:var(--text-muted);font-size:0.8rem;margin-top:4px">"{{ $todayMood->note }}"</div>
                @endif
                <div style="font-size:0.75rem;color:var(--text-muted);margin-top:8px">Mood logged for today ✓</div>
            </div>
            @else
            <form action="{{ route('mood.store') }}" method="POST" id="solo-mood-form">
                @csrf
                <div style="display:flex;gap:10px;justify-content:center;margin-bottom:16px">
                    ...
                    <button type="button"
                        class="mood-btn"
                        data-level="{{ $level }}"
                        data-target="solo_mood_level"
                        onclick="selectMood(this)">{{ $emoji }}</button>
                    @endforeach
                </div>
                <input type="hidden" name="mood_level" id="solo_mood_level" value="3">
...
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;margin-bottom:20px">
    ...
    <div class="card">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
            <div style="text-align:center">
                ...
                <button onclick="openMoodModal()" style="font-size:0.7rem;color:#8b5cf6;background:none;border:none;cursor:pointer;padding:0">Log mood</button>
                @endif
            </div>
            <div style="text-align:center">
    ...
    <!-- Next Special Date — fully null-safe -->
    <div class="card">
...
{{-- ============ MOOD MODAL (works for both solo & couple) ============ --}}
@if(!$todayMood)
<div id="mood-modal" class="mood-overlay" onclick="if(event.target===this) closeMoodModal()">
    <div class="card" style="max-width:400px;width:calc(100% - 40px);margin:20px">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
            <h3 style="font-weight:700;margin:0">How are you feeling? 💫</h3>
            <button onclick="closeMoodModal()" style="background:none;border:none;font-size:1.2rem;cursor:pointer;line-height:1">✕</button>
        </div>
        <form action="{{ route('mood.store') }}" method="POST" id="modal-mood-form">
            @csrf
            <div style="display:flex;gap:12px;justify-content:center;margin-bottom:16px">
                ...
                <button type="button"
                    class="mood-btn"
                    data-level="{{ $level }}"
                    data-target="modal_mood_level"
                    onclick="selectMood(this)">{{ $emoji }}</button>
                @endforeach
            </div>
            <input type="hidden" name="mood_level" id="modal_mood_level" value="3">
...
// Shared mood button selector — works for both solo widget & modal
function selectMood(btn) {
    const targetId = btn.dataset.target;
    // Deselect siblings in same group
    btn.closest('div').querySelectorAll('.mood-btn').forEach(b => b.classList.remove('selected'));
    btn.classList.add('selected');
    // Update hidden input
    const input = document.getElementById(targetId);
    if (input) input.value = btn.dataset.level;
}

// Pre-select default mood (level 3) on page load for each form
document.addEventListener('DOMContentLoaded', function() {
    ['solo_mood_level', 'modal_mood_level'].forEach(function(id) {
        const input = document.getElementById(id);
        if (!input) return;
        const defaultBtn = document.querySelector(
            `.mood-btn[data-target="${id}"][data-level="3"]`
        );
